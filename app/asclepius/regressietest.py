...
class TestExcel:
    ...
    def join_excels(self, excel_a, excel_p) -> DataFrame:
        ...
        del_columns = []
        for i, column in enumerate(columns_a): 
        ...
        columns_a = [columns_a[i] for i in range(len(columns_a)) if i not in del_columns]
        
        del_columns = []
        for i, column in enumerate(columns_p):
            if column == self.join_col:
                pass
            elif column not in self.test_cols:
        ...
        columns_p = [columns_p[i] for i in range(len(columns_p)) if i not in del_columns]
        ...
        self.columns_a = data_a.columns.values.tolist()
        self.columns_p = data_p.columns.values.tolist()
